...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20, Initializable {
    ...
    function addLiquidity(
        ...
        if(!firstLiquidityAdded){
            ...
            (bool success, bytes memory data) = adventurerSFTAddress.call(abi.encodeWithSelector(bytes4(keccak256("completionCheck(address,string,uint256)")), msg.sender, projectId, version));
            require(success, "UniswapDComp: UNSUCCESSFUL_CALL");
            success = abi.decode(data, (bool));
            require(success, "UniswapDComp: NOT_PROJECT_HOLDER");
        }
    ...
    function setAdventurerSFTAddr(address _adventurerSFTAddr) external {
        adventurerSFTAddress = _adventurerSFTAddr;
    }
}